...
It is a built in button that helps you change the type of input from password to text, so user can see the password. Having this button is pretty simple you just need to add a `<button>` with `.show-password-button` class inside the `.input-wrapper` element.
