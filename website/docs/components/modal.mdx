...
There is also a CSS variable called `--flatify__modal-backdrop-color`, it just changes the color of the modal backdrop. The important thing is that is not inside modal, it is a global element at the first level of body children, so you must set its custom color to the `:root`, `html` or `body`.
:::
