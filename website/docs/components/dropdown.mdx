...
To create a simple dropdown, we need a [button](docs/components/button) (specifically an [arrow button](/docs/components/buttons/arrow-button)) with `.dropdown-toggle` class, [menu](docs/components/menu) with `.dropdown` class, also they should be wrapped by `.dropdown-wrapper` class. These are basics and they should have some special classes to work that you can see in the example below.
