...
In the first example on click, there is just a loading component inside the button, and button width and height changed based on it, so layout shifts and it is the bad user experience we were talking about.  
Otherwise second example use [overlay layer helper class](/docs/helpers/overlay-layer), adds `.overlay-layer` class to the button and appends `<span class="loading"></span>` inside button next to button text.  
Technically it creates an overlay layer that hides button content and just shows loading, it will prevent unwanted layout shifts and changes will be done smoothly!
