...
In the first example on click, there is just a loading component inside the button, and button width and height change based on it, so layout shifts and it is the bad user experience we were talking about.  
...
Technically it creates an overlay element with CSS `::before` that hides button content and just shows loading element, it will prevent unwanted layout shifts and changes will be done smoothly!
:::
...
`--flatify__loading-dots-animation-duration`  
	...
	<head>
		...
		<style>
			...
			.my-dots-loading-1{
				...
				--flatify__loading-dots-animation-duration: 0.75s;
			}
			.my-dots-loading-2{
				--flatify__loading-color: #BF263C;
				--flatify__loading-dot-width: 0.5em;
				--flatify__loading-dot-height: 2em;
				--flatify__loading-dot-animation-start-scale: 2;
				--flatify__loading-dots-animation-duration: 2s;
			}
		</style>
	</head>
	<body class="flex-center">
