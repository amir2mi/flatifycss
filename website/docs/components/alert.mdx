...
2- Change the delay that the alert after it will get removed by JS in the `config.json` file, change `alertRemoveDelay` to whatever you want however it is suggested setting this delay duration more than CSS animation to prevent an immediate shift in UI.
:::
