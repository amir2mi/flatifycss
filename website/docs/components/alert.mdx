...
Any HTML tag can be alert with the `.alert` class,
...
Like any other component, alerts can have different colors
by adding [color setter classes](/docs/helpers/color-setters).

:::success
Alerts can have the light theme to gain less attention since you can set color setter helper class
with `-light` suffix, e.g. `style-danger-light`.
:::
...
It is possible to have an SVG as an icon for alert just add `.has-icon` to the alert and put
SVG inside a div with `.alert-svg` class before message.
...
Each alert has these CSS variables for customization, they can be set as `:root` for all alerts default style
...
```css title="style.css"
/* Define the custom animation when alert is going to be removed */
@keyframes my-custom-animation {
	from {
		opacity: 1;
		transform: scale(1);
...
To change alert remove animation duration you need to change two things:

1- Change `$ALERT_REMOVE_ANIMATION` in `_config.scss` or `--flatify__alert-remove-animation` CSS variable.  
2- Change the delay that the alert after it will get removed by JS in the `config.json` file,
change `alertRemoveDelay` to whatever you want however it is suggested setting this delay duration
more than CSS animation to prevent an immediate shift in UI.
:::
