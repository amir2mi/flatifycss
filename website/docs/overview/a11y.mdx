...
FlatifyCSS takes car about this fact and modify these from `a11y.scss` file:
...
We use `flatify-fade-in`, `flatify-fade-in-out` and `flatify-fade-out` animations instead of primary, also it is a good pratice to reduced animation steps for complex animations like what we do for [loadings](/docs/components/loading):
