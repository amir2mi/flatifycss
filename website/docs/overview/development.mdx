...
FlatifyCSS is written in [Sass](https://sass-lang.com/), which we compile to CSS using Gulp tasks, in this page we will cover the development process.
...
There are two files in this directory: `flatify.scss` and `flatify-rtl.scss`. The `flatify.scss` file is the main file we use to extend the base stylesheet, and the `flatify-rtl.scss` file is used to compile stylesheets for RTL.
